...
public class VideoUrlService {
    ...
    /**
     * Only platform-managed object keys need HLS transcoding. External course
     * providers keep ownership of their HTTP(S) media and are played directly.
     */
    public boolean requiresTranscode(String rawUrl) {
        if (rawUrl == null || rawUrl.isBlank()) {
            return false;
        }
        String normalized = rawUrl.trim().toLowerCase(java.util.Locale.ROOT);
