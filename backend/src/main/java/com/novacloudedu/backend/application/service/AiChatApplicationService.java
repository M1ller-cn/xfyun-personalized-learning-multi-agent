...
public class AiChatApplicationService {
    ...
    public Map<String, Object> assistantStreamChat(Long assistantId, Long userId, Long sessionId,
        ...
        // 使用助手配置的模型和参数
        String modelId = normalizeAssistantModelId(modelConfig != null ? modelConfig.getModelName() : null);
    ...
    private String normalizeAssistantModelId(String modelId) {
        if (modelId == null || modelId.isBlank()) {
            return "deepseek/deepseek-chat";
        }
        String normalized = modelId.trim();
        if (normalized.equalsIgnoreCase("qwen-plus")
                || normalized.equalsIgnoreCase("dashscope/qwen-plus")
                || normalized.equalsIgnoreCase("deepseek-v4-pro")
                || normalized.equalsIgnoreCase("deepseek/deepseek-v4-pro")) {
            return "deepseek/deepseek-chat";
        }
