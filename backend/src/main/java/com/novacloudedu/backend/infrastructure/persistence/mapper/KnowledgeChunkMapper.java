...
public interface KnowledgeChunkMapper extends BaseMapper<KnowledgeChunkPO> {
    ...
    @Select("<script>" +
            "<![CDATA[" +
            "SELECT id, knowledge_base_id, document_id, content, metadata, 0.0 as similarity " +
            "FROM knowledge_chunk " +
            "WHERE knowledge_base_id IN " +
            "]]>" +
            "<foreach item='id' collection='knowledgeBaseIds' open='(' separator=',' close=')'>" +
            ...
            "]]>" +
            "</script>")
    List<Map<String, Object>> listSearchCandidatesInMultiple(@Param("knowledgeBaseIds") List<Long> knowledgeBaseIds,
