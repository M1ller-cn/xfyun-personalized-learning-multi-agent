...
LOCAL_STORAGE_DIR = Path(os.environ.get("PPT_LOCAL_STORAGE_DIR", "/app/data/outputs"))
PPT_PUBLIC_BASE_URL = os.environ.get("PPT_PUBLIC_BASE_URL", "http://127.0.0.1:8100").rstrip("/")
...
def get_local_storage_dir() -> Path:
    """Return the persistent local output directory used when OSS is unavailable."""
...
def upload_bytes(
    ...
    # A local demo should not require cloud storage credentials just to make a
    # downloadable presentation. Keep the OSS path when configured, otherwise
    # persist the output in the service's mounted local volume.
    if not _is_configured():
        output_path = get_local_storage_dir() / object_name
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_bytes(data)
        url = f"{PPT_PUBLIC_BASE_URL}/files/{object_name}"
        logger.info(
            "OSS is not configured; saved PPT artifact locally: %s (%.1f KB)",
            output_path, len(data) / 1024)
        return url
