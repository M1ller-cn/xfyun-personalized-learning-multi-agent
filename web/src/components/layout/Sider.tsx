...
import { BookMarked, Bot, BrainCircuit, ChevronsLeft, ChevronsRight, GraduationCap, Home, LayoutGrid, MessageCircle, PenTool } from 'lucide-react';
...
export const Sider: React.FC<SiderProps> = ({ collapsed: propCollapsed = false }) => {
  ...
  const textFadeClass = `whitespace-nowrap overflow-hidden transition-opacity ${isCollapsed ? 'opacity-0 w-0 duration-200' : 'opacity-100 duration-200 delay-200'}`;
  ...
  const navItems: NavItem[] = [
    { label: '首页', path: '/', icon: <Home size={15} />, active: location.pathname === '/' },
    { label: '圈子', path: '/circle', icon: <LayoutGrid size={15} />, active: location.pathname.startsWith('/circle') },
    { label: '聊天', path: '/chat', icon: <MessageCircle size={15} />, active: location.pathname.startsWith('/chat') },
    { label: '书架', path: '/bookshelf', icon: <BookMarked size={15} />, active: location.pathname.startsWith('/bookshelf') },
    { label: '课程', path: '/courses', icon: <GraduationCap size={15} />, active: location.pathname === '/courses' },
    { label: '学习工作台', path: '/learning-workspace', icon: <BrainCircuit size={15} />, active: location.pathname.startsWith('/learning-workspace') },
    { label: '智能批改', path: '/grading', icon: <PenTool size={15} />, active: location.pathname.startsWith('/grading') },
  ];

  const activeAssistantId = location.pathname.startsWith('/ai-chat')
    ? location.pathname.split('/ai-chat/')[1]?.split('/')[0]
    : undefined;
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-5">
        <div>
          ...
          <ul className="space-y-1">
            {navItems.map((item) => (
              <li key={item.path}>
                <SiderTooltip label={item.label} show={isCollapsed}>
                  <button
                  ...
                  >
                    ...
                    <span className={textFadeClass}>{item.label}</span>
                  </button>
                </SiderTooltip>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-1">
            <li>
              <SiderTooltip label="智能体中心" show={isCollapsed}>
                <button
                  onClick={() => navigate('/chat?tab=intelligence')}
                  className={`w-full flex items-center ${isCollapsed ? 'justify-center px-0 gap-0' : 'gap-3 px-3'} py-2 text-sm text-gray-700 dark:text-gray-300 rounded-lg hover:bg-brand-50 dark:hover:bg-brand-900/30 hover:text-brand-600 dark:hover:text-brand-400 transition-all duration-200`}
                >
                  <div className="w-7 h-7 rounded-lg flex-shrink-0 bg-brand-500 text-white flex items-center justify-center shadow-sm">
                    <Bot size={15} />
                  </div>
                  <span className={`font-medium ${textFadeClass}`}>智能体中心</span>
                </button>
              </SiderTooltip>
            </li>
            {aiLoading
                      ...
                      <SiderTooltip label={assistant?.name || '加载中'} show={isCollapsed}>
                        ...
                        >
                          <div className={`w-7 h-7 rounded-full flex-shrink-0 flex items-center justify-center text-xs transition-colors ${
                            ...
                            {assistant?.name ? assistant.name[0] : 'AI'}
                          </div>
