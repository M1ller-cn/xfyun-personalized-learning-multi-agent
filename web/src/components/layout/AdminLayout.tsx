...
const TEACHER_ALLOWED_PATHS = new Set([
  '/admin',
  '/admin/courses',
  '/admin/classes',
...
const isTeacherPathAllowed = (pathname: string) =>
  TEACHER_ALLOWED_PATHS.has(pathname) || pathname.startsWith('/admin/courses/');
...
const AdminSider: React.FC<AdminSiderProps> = ({ 
  ...
  const allMenuItems = [
    ...
    { icon: BarChart3, label: '学情分析', path: '/admin/learning-analytics' },
    ...
    { icon: Activity, label: '平台就绪检查', path: '/admin/platform-readiness' },
  ];

  // 教师只显示指定菜单，管理员显示全部
  const menuItems = userRole === 'teacher'
    ? allMenuItems.filter(item => TEACHER_ALLOWED_PATHS.has(item.path))
    : allMenuItems;
...
export const AdminLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  React.useEffect(() => {
    ...
    const role = String(info?.userRole || info?.role || '').toLowerCase();
    if (role === 'teacher' && !isTeacherPathAllowed(location.pathname)) {
