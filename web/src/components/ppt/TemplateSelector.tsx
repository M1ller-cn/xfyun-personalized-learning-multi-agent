        ...
        <div className="flex-1 overflow-y-auto p-6">
          {showSkipPanel ? (
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center justify-between gap-3"><div><h4 className="font-semibold text-gray-900 dark:text-white">系统内置生成风格</h4><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">不依赖上传文件，AI 会按所选风格设计整套演示文稿。</p></div><span className="text-xs font-medium text-brand-600 dark:text-brand-400">可直接使用</span></div>
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  {STYLE_PRESETS.map((preset) => (
                    <button key={preset.label} type="button" onClick={() => { onClose(); onSkip?.(preset.hint); }} className="rounded-xl border border-gray-200 bg-white px-3 py-3 text-left transition-colors hover:border-brand-400 hover:bg-brand-50/40 dark:border-gray-700 dark:bg-gray-800 dark:hover:border-brand-500 dark:hover:bg-brand-950/20"><p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{preset.label}</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-gray-500 dark:text-gray-400">{preset.hint}</p></button>
                  ))}
                </div>
              </div>
              <div className="border-t border-gray-100 pt-5 dark:border-gray-800"><h4 className="font-semibold text-gray-900 dark:text-white">上传的 PPT 模板</h4><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">选择已解析的 PPTX 模板，让 AI 参考其版式生成。</p></div>
          {loading ? (
            <div className="flex items-center justify-center h-32">
              <Loader2 className="w-8 h-8 animate-spin text-brand-500" />
            </div>
          ) : templates.length === 0 ? (
            <div className="rounded-xl bg-gray-50 px-4 py-7 text-center text-gray-400 dark:bg-gray-800/50">
              <p className="text-sm font-medium">暂时没有上传模板</p>
              <p className="mt-1 text-xs">可先使用上方内置风格，或在模板管理页上传 PPTX。</p>
            </div>
          ) : (
