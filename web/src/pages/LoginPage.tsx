...
const getRoleEntry = (role?: string | null) => {
  const normalized = String(role || '').toLowerCase();
  return normalized === 'admin' || normalized === 'teacher' ? '/admin' : '/';
};
...
export default function LoginPage() {
  ...
  // A role-specific entry is used by the visitor portal and demo script. It must
  // show credentials even when the browser retains a previous recording session.
  const forceCredentialPrompt = entry === 'teacher' || entry === 'student';
  ...
  useEffect(() => {
    ...
    if (getToken() && !forceCredentialPrompt) {
      ...
      navigate(getRoleEntry(userInfo?.userRole || userInfo?.role), { replace: true });
    }
  }, [forceCredentialPrompt, navigate]);
  ...
  const handleLoginSuccess = (data: any) => {
    if (data?.data?.token) {
      ...
      toast.success('登录成功');
      navigate(getRoleEntry(userInfo.userRole), { replace: true });
    }
  };
  ...
  }, [account, password, entry, navigate]);
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-20 h-20 bg-white dark:bg-gray-800 rounded-3xl shadow-xl shadow-brand-500/10 mb-6 rotate-3 hover:rotate-0 transition-transform duration-300 overflow-hidden p-4">
              <img src={logo} alt="星图智课" className="w-full h-full object-contain" />
            </div>
