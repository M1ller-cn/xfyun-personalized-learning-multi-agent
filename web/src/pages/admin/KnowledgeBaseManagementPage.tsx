...
type KnowledgeBaseStarter = { name: string; description: string; course: string; documents: string; chunks: string; color: string };

const KNOWLEDGE_BASE_STARTERS: KnowledgeBaseStarter[] = [
  { name: 'CS61B 数据结构课程知识库', description: '面向外教课程的中文知识导学：覆盖哈希表、树、图、复杂度与项目资料。', course: '数据结构与算法', documents: '讲义 · 字幕 · 作业解析', chunks: '语义切分 + Rerank', color: 'bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300' },
  { name: 'CSAPP 计算机系统导论知识库', description: '聚合二进制、汇编、存储层次与链接的课程资料，供智能辅导精准引用。', course: '计算机系统', documents: '教材章节 · 课堂笔记 · 实验说明', chunks: '标题切分 + 混合召回', color: 'bg-violet-50 text-violet-700 dark:bg-violet-950/30 dark:text-violet-300' },
];

const createDefaultKBForm = (starter?: KnowledgeBaseStarter | null): KBFormData => ({
  name: starter?.name || '', description: starter?.description || '', embeddingModel: 'text-embedding-v4', embeddingDimension: 1024,
  chunkSize: 500, chunkOverlap: 50, chunkStrategy: 'SEMANTIC', parentChildMode: false, parentChunkSize: 1500,
  preserveMetadata: true, semanticThreshold: 0.5, retrievalMode: 'HYBRID_RERANK', enableQueryRewrite: false,
  useDynamicTopK: true, defaultTopK: 5, queryRewriteModelId: 'dashscope/qwen-turbo', rerankModel: 'qwen3-rerank',
});
...
interface KBFormModalProps {
  ...
  starter?: KnowledgeBaseStarter | null;
}

const KBFormModal: React.FC<KBFormModalProps> = ({ isOpen, onClose, onSuccess, knowledgeBase, starter }) => {
  const isEdit = !!knowledgeBase;
  const [loading, setLoading] = useState(false);
  const [showAdvancedChunk, setShowAdvancedChunk] = useState(false);
  const [formData, setFormData] = useState<KBFormData>(createDefaultKBForm());
  ...
  useEffect(() => {
    if (knowledgeBase) {
      ...
      setFormData(createDefaultKBForm(starter));
    }
    setShowAdvancedChunk(false);
  }, [knowledgeBase, isOpen, starter]);
...
export const KnowledgeBaseManagementPage: React.FC = () => {
  ...
  const [starterPreset, setStarterPreset] = useState<KnowledgeBaseStarter | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => { setEditingKB(null); setStarterPreset(null); setModalOpen(true); }}
            className="flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-xl text-sm font-bold hover:bg-brand-700 shadow-lg shadow-brand-600/20 transition-all active:scale-95"
          >
            <Plus size={18} />
            <span>新建知识库</span>
      ...
      <section className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-base font-bold text-gray-900 dark:text-white">课程知识库示例</h2><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">选择一个内置配置后，系统会带入检索策略与字段；创建后再上传课程资料即可投入使用。</p></div><span className="rounded-md bg-brand-50 px-2 py-1 text-xs font-semibold text-brand-700 dark:bg-brand-950/30 dark:text-brand-300">2 个可用示例</span></div>
        <div className="mt-4 grid gap-4 lg:grid-cols-2">{KNOWLEDGE_BASE_STARTERS.map((starter) => <div key={starter.name} className="rounded-xl border border-gray-100 p-4 dark:border-gray-800"><div className="flex items-start justify-between gap-3"><div><span className={`inline-flex rounded-md px-2 py-1 text-xs font-semibold ${starter.color}`}>{starter.course}</span><h3 className="mt-3 font-bold text-gray-900 dark:text-white">{starter.name}</h3><p className="mt-1.5 text-sm leading-6 text-gray-500 dark:text-gray-400">{starter.description}</p></div><Database className="shrink-0 text-brand-500" size={22} /></div><div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-3 text-xs text-gray-500 dark:border-gray-800 dark:text-gray-400"><span>{starter.documents}</span><span>{starter.chunks}</span><button type="button" onClick={() => { setEditingKB(null); setStarterPreset(starter); setModalOpen(true); }} className="font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400">使用此示例</button></div></div>)}</div>
      </section>
      ...
      <KBFormModal
        isOpen={modalOpen}
        onClose={() => { setModalOpen(false); setEditingKB(null); setStarterPreset(null); }}
