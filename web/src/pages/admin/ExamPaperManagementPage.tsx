...
const SYSTEM_EXAM_TEMPLATE_PRESETS = [
  { id: 'standard', name: '标准课程测验', note: '单栏排版 · 适合章节小测', accent: 'bg-blue-500' },
  { id: 'midterm', name: '期中综合测评', note: '分区清晰 · 适合笔试与诊断', accent: 'bg-violet-500' },
  { id: 'coding', name: '代码实训任务', note: '代码题优先 · 含样例说明区', accent: 'bg-emerald-500' },
  { id: 'review', name: '考前冲刺卷', note: '难度分层 · 适合限时练习', accent: 'bg-amber-500' },
];

const DEMO_EXAM_PAPERS = [
  { title: 'CS61B 哈希表与树章节测验', subject: '数据结构与算法', score: 100, duration: 45, status: '已发布', template: '标准课程测验', note: '126 人已收到 · 79 人完成代码评测' },
  { title: 'CSAPP 第二章数据表示诊断', subject: '计算机系统导论', score: 80, duration: 40, status: '草稿', template: '期中综合测评', note: '覆盖整数表示、位运算与浮点数' },
  { title: 'Python 递归与高阶函数补救练习', subject: '程序设计方法', score: 60, duration: 30, status: '已发布', template: '代码实训任务', note: '面向 21 名需要补学的学生' },
];

const mergeTemplateIntoLayout = (layout: string, templateId: string) => {
  try {
    const parsed = layout ? JSON.parse(layout) : {};
    return JSON.stringify({ ...parsed, systemTemplate: templateId || undefined });
  } catch {
    return JSON.stringify({ systemTemplate: templateId || undefined });
  }
};
...
export const ExamPaperManagementPage: React.FC = () => {
  ...
  const [selectedSystemTemplate, setSelectedSystemTemplate] = useState('standard');
  ...
  const handleCreate = () => {
    ...
    setSelectedSystemTemplate('standard');
    setShowModal(true);
  };

  const handleUseDemoPaper = (paper: typeof DEMO_EXAM_PAPERS[number]) => {
    setEditingId(null);
    setSelectedSystemTemplate(SYSTEM_EXAM_TEMPLATE_PRESETS.find((template) => template.name === paper.template)?.id || 'standard');
    setForm({ title: paper.title, subtitle: `${paper.subject} · ${paper.score} 分`, subject: 'MATH', grade: '本科', durationMin: paper.duration, layout: '{}', templateId: '' });
    setShowModal(true);
  };
  ...
  const handleEdit = (p: ExamPaperResponse) => {
    ...
    try { setSelectedSystemTemplate(JSON.parse(p.layout || '{}')?.systemTemplate || 'standard'); } catch { setSelectedSystemTemplate('standard'); }
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const layout = mergeTemplateIntoLayout(form.layout, selectedSystemTemplate);
      if (editingId) {
          ...
          <div className="space-y-3">
            {loading && papers.length === 0 ? (
              ...
              <div className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
                <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="font-bold text-gray-900 dark:text-white">示例试卷</h2><span className="rounded-md bg-brand-50 px-2 py-1 text-xs font-semibold text-brand-700 dark:bg-brand-950/30 dark:text-brand-300">演示数据</span></div><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">可直接带入标题、时长和模板，继续添加题目后发布给班级。</p></div><button type="button" onClick={handleCreate} className="text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400">空白新建</button></div>
                <div className="mt-4 grid gap-3 xl:grid-cols-3">{DEMO_EXAM_PAPERS.map((paper) => <div key={paper.title} className="rounded-xl border border-gray-100 p-4 dark:border-gray-800"><div className="flex items-start justify-between gap-3"><div><span className={`rounded-md px-2 py-1 text-xs font-semibold ${paper.status === '已发布' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-300'}`}>{paper.status}</span><h3 className="mt-3 font-bold text-gray-900 dark:text-white">{paper.title}</h3></div><FileText className="shrink-0 text-brand-500" size={20} /></div><div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-gray-400"><span>{paper.subject}</span><span>{paper.score} 分</span><span>{paper.duration} 分钟</span></div><p className="mt-2 text-xs leading-5 text-gray-400">{paper.note}</p><div className="mt-4 border-t border-gray-100 pt-3 text-xs dark:border-gray-800"><span className="text-gray-400">{paper.template}</span><button type="button" onClick={() => handleUseDemoPaper(paper)} className="float-right font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400">基于此试卷新建</button></div></div>)}</div>
              </div>
            ) : papers.map(p => (
              <div key={String(p.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg mx-4 animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between gap-3"><label className="block text-sm font-medium text-gray-700 dark:text-gray-300">系统内置模板</label><span className="text-xs text-gray-400">选择后会随试卷保存</span></div>
                <div className="mt-2 grid grid-cols-2 gap-2">{SYSTEM_EXAM_TEMPLATE_PRESETS.map((template) => <button type="button" key={template.id} onClick={() => { setSelectedSystemTemplate(template.id); setForm((current) => ({ ...current, templateId: '' })); }} className={`rounded-lg border p-3 text-left transition-colors ${selectedSystemTemplate === template.id && !form.templateId ? 'border-brand-500 bg-brand-50/60 dark:bg-brand-950/20' : 'border-gray-200 bg-white hover:border-brand-300 dark:border-gray-700 dark:bg-gray-800'}`}><span className={`mb-2 block h-1.5 w-10 rounded-full ${template.accent}`} /><p className="text-sm font-semibold text-gray-800 dark:text-gray-100">{template.name}</p><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{template.note}</p></button>)}</div>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">上传的排版模板</label>
                <select value={form.templateId} onChange={e => { setForm(f => ({ ...f, templateId: e.target.value })); if (e.target.value) setSelectedSystemTemplate(''); }}
                  className="w-full px-4 py-2.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 transition-all cursor-pointer">
                  <option value="">不使用上传模板</option>
