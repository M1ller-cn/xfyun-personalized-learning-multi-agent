...
const BUILT_IN_PPT_STYLES = [
  { name: '智课蓝图', description: '蓝白理性布局，适合课程汇报与教学数据展示', tone: 'bg-blue-50 text-blue-700 dark:bg-blue-950/30 dark:text-blue-300' },
  { name: '学术论文', description: '清晰章节层级，图表与引用优先，适合学术汇报与研究展示', tone: 'bg-indigo-50 text-indigo-700 dark:bg-indigo-950/30 dark:text-indigo-300' },
  { name: '科技叙事', description: '深色舞台与数据可视化，适合项目路演和产品介绍', tone: 'bg-cyan-50 text-cyan-700 dark:bg-cyan-950/30 dark:text-cyan-300' },
  { name: '课堂导学', description: '重点突出、页间节奏明快，适合中文课程讲解', tone: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-300' },
];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <section className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-base font-bold text-gray-900 dark:text-white">系统内置生成风格</h2><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">无需上传 PPTX，生成助手会根据所选风格规划版式、配色和页面节奏。</p></div><a href="/admin/ppt-generator" className="rounded-lg bg-brand-600 px-3 py-2 text-sm font-semibold text-white transition-colors hover:bg-brand-700">前往生成助手</a></div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{BUILT_IN_PPT_STYLES.map((style) => <div key={style.name} className="rounded-xl border border-gray-100 p-4 dark:border-gray-800"><span className={`inline-flex rounded-md px-2 py-1 text-xs font-semibold ${style.tone}`}>系统内置</span><h3 className="mt-3 font-bold text-gray-900 dark:text-white">{style.name}</h3><p className="mt-1.5 text-sm leading-6 text-gray-500 dark:text-gray-400">{style.description}</p><a href="/admin/ppt-generator" className="mt-4 inline-flex text-sm font-semibold text-brand-600 hover:text-brand-700 dark:text-brand-400">在生成助手中使用</a></div>)}</div>
      </section>
